...
/** 单格的最终绘制内容 */
type Cell = { bg: string; kind: 'color'; text: string; } | { img: HTMLImageElement; kind: 'image'; }
...
function computeCellRectsMedium(count: number, target: number, divider: number): CellRect[] {
  ...
  const step = s + divider
  const xs: [number, number, number] = [divider, divider + step, divider + 2 * step]
  // 2 行高 + 1 行间 div
...
function computeCellRectsLarge(count: number, target: number, divider: number): CellRect[] {
  ...
  const step = s + divider
  const xs: [number, number, number] = [divider, divider + step, divider + 2 * step]
  const ys: [number, number, number] = [divider, divider + step, divider + 2 * step]
  const rects: CellRect[] = []
  if (count === 7) {
    // 上 1 居中 + 中 3 + 下 3
