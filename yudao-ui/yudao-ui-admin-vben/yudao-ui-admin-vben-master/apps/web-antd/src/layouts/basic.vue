...
import { AuthenticationLoginExpiredModal, useVbenModal, VbenButton } from '@vben/common-ui';
...
// 打开 AI 聊天（新标签页，独立无后台布局）
function handleOpenAi() {
  const { href } = router.resolve({ path: '/ai/chat-popup' });
  window.open(href, '_blank');
}
    ...
    <template #header-right-900>
      <Tooltip title="AI助手">
        <button
          ...
          @click="handleOpenAi"
        >
           <IconifyIcon class="size-4" icon="tabler:ai" />
        </button>
      </Tooltip>
    </template>
    <template #extra>
